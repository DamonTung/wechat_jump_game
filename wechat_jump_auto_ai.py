...
def set_button_position(im):
    ...
    # left = int(random.uniform(left - 30, left + 30))
    # top = int(random.uniform(top - 30, top + 30))  # 随机防 ban
    after_top = int(random.uniform(top - 30, top + 30))
    after_left = int(random.uniform(left - 30, left + 30))
    ...
    while True:
        ...
        if i == next_rest:
            ...
            for j in range(next_rest_time):
                ...
                time.sleep(0.5)
            print('\n继续')
            i, next_rest, next_rest_time = 0, random.randrange(30, 100), random.randrange(1, 6)
        time.sleep(random.uniform(0.0, 1.5))  # 为了保证截图的时候应落稳了，多延迟一会儿，随机值防 ban
